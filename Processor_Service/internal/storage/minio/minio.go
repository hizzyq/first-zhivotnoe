...
func (s *Storage) UploadMedia(ctx context.Context, objectName, contentType string, size int64, reader io.Reader) (string, error) {
	const op = "storage.minio.UploadMedia"
...
func (s *Storage) DownloadMedia(ctx context.Context, objectName string) (io.ReadCloser, error) {
	const op = "storage.minio.DownloadMedia"

	obj, err := s.client.GetObject(ctx, s.bucketName, objectName, minio.GetObjectOptions{})
	if err != nil {
		return nil, fmt.Errorf("%s: error while getting object: %w", op, err)
	}

	_, err = obj.Stat()
	if err != nil {
		obj.Close()
		return nil, fmt.Errorf("%s: object stat failed (file may not exist): %w", op, err)
	}
